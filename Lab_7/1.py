	...
	def search_account_from_card(self, card_no):
		for user in self.__user_list:
			for account in user.get_account_list:
				if account.get_card() and account.get_card().get_card_no == card_no:
					return account
		return None
...
class SavingAccount(Account):
	...
	def deposit(self, account_no, amount):
		if amount > 0:
			self.set_amount(self.get_amount + amount)
			self.get_transactions.append(
				Transaction("D", amount, self.get_amount, None)
			)
			return "Success"
		else:
	...
	def withdraw(self, account_no, amount):
		if 0 < amount <= self.get_amount:
			self.set_amount(self.get_amount - amount)
			self.__transactions.append(
				Transaction("W", amount, self.get_amount, None)
            )
			return "Success"
		else:
			return "ERROR Invalid"

	def transfer(self, account_no, amount, target_account):
		if 0 < amount <= self.get_amount:
			self.set_amount(self.get_amount - amount)
			target_account.set_amount(target_account.get_amount + amount)
			self.get_transactions.append(
				Transaction("T", amount, self.get_amount, target_account.get_account_no)
            )
			target_account.get_transactions.append(
				Transaction("T", amount, target_account.get_amount, self.get_account_no)
            )
			return "Success"
		else:
			return "ERROR Invalid"
...
class Transaction:
	...
	def __str__(self):
		return f"Type: {self.__transaction_type}, Amount: {self.__amount}, Total: {self.__total}, Target Account: {self.__target_account}"
...
class ATM_machine:
	...
	def deposit(self, account, amount):
		if amount > 0:
			account.set_amount(account.get_amount + amount)
			account.get_transactions.append(
				Transaction("D", amount, account.get_amount, None)
            )
			self.__money += amount
			return "Success"
		else:
	...
	def withdraw(self, account, amount):
		if 0 < amount <= account.get_amount:
			account.set_amount(account.get_amount - amount)
			account.get_transactions.append(
				Transaction("W", amount, account.get_amount, None)
            )
			self.__money -= amount
			return "Success"
		else:
	...
	def transfer(self, account, amount, target_account):
		if 0 < amount <= account.get_amount:
			...
			target_account.set_amount(target_account.get_amount + amount)
			account.get_transactions.append(
				Transaction("T", amount, account.get_amount, target_account.get_account_no)
            )
			target_account.get_transactions.append(
				Transaction("T", amount, target_account.get_amount, account.get_account_no)
            )
			return "Success"
		else:
			return "ERROR Invalid"
...
class Seller:
	...
	def paid(self, account, amount, seller_account):
		if 0 < amount <= account.get_amount:
			account.set_amount(account.get_amount - amount)
			seller_account.set_amount(seller_account.get_amount + amount)
			account.get_transactions.append(Transaction("P", amount, account.get_amount, seller_account.get_account_no))
			seller_account.get_transactions.append(Transaction("P", amount, seller_account.get_amount, account.get_account_no))
			return "Success"
		else:
			return "ERROR Invalid"
...
class EDC_machine:
	...
	def paid(self, card, amount, account):
		if 0 < amount <= card.get_account.get_amount:
			card.get_account.set_amount(card.get_account.get_amount - amount)
			account.set_amount(account.get_amount + amount)
			card.get_account.get_transactions.append(Transaction("P", amount, card.get_account.get_amount, account.get_account_no))
			account.get_transactions.append(Transaction("P", amount, account.get_amount, card.get_account.get_account_no))
			return "Success"
		else:
			return "ERROR Invalid"
