...
class Bank:
    ...
    def create_card(self, account, card_number, pin):
        card = AtmCard(card_number, pin)
...
class Account:
    def __init__(self, account_number, balance, card):
    ...
    def deposit(self, atm, account, amount):
        if amount > 0:
            ...
            self.__transactions.append(
                Transaction("D", amount, None, atm.get_atm_number, self.__balance)
            )
    ...
    def withdraw(self, atm, account, amount):
        if 0 < amount <= self.__balance:
            self.__balance -= amount
            self.__transactions.append(
                Transaction("W", amount, None, atm.get_atm_number, self.__balance)
            )
            atm.set_atm_balance(atm.get_atm_balance - amount)
            return "Success"
        else:
...
user = {
    "1-1101-12345-12-0": ["Harry Potter", "1234567890", 20000, "12345"],
...
def add_instance(user_data, atm_data):
    ...
    for citizen_id, user_info in user_data.items():
        name, account_number, balance, card_number = user_info
        new_user = bank.insert_user(citizen_id, name)
        new_account = bank.create_account(
            new_user, account_number, balance, card_number
        )
        pin = user_info[3]
        new_card = bank.create_card(new_account, card_number, pin)
...
print("Test case #1")
authenticated_account_harry = bank_instance.get_atms[0].authenticate(
    bank_instance,
    bank_instance.get_users[0].get_accounts[0].get_card,
    "12345"
)
if authenticated_account_harry:
    print(f"Authenticated Account for Harry: {authenticated_account_harry.get_account_number}")
else:
...
print("Test case #3")
result_negative_deposit = hermione_account.deposit(
    bank_instance.get_atms[1], hermione_account, -1
)
...
result_withdrawal = hermione_account.withdraw(
    bank_instance.get_atms[1], hermione_account, 500
)
print(f"Hermione account after test: {hermione_account.get_balance}")
...
# ผลที่คาดหวัง : แสดง Error
print("test case #5")
result_over_withdrawal = hermione_account.withdraw(
    bank_instance.get_atms[1], hermione_account, 2000
)
...
print(f"Hermione account before test: {hermione_account.get_balance}")
result_transfer = harry_account.transfer(
    bank_instance.get_atms[1], harry_account, hermione_account, 10000
)
